...
def collect_recent_results(
    leagues: list[dict], days_back: int, already_posted_ids: set[str] = frozenset(), on_progress=None,
) -> list[dict]:
    """`already_posted_ids` — ilgari joylangan o'yinlarning `idEvent`lari (bot_hub
    orqali; qarang: state.py). Bu ID'ga ega o'yinlar NATIJALAR RO'YXATIGA umuman
    kiritilmaydi — shu bilan ularning gol jadvalini ham qayta so'rab, vaqt/so'rov
    isrof qilinmaydi (chunki baribir qayta joylanmaydi)."""
    today = date.today()
    results = []
    for league in leagues:
        try:
            ...
            for ev in events:
                if not ev.get("dateEvent") or str(ev.get("idEvent")) in already_posted_ids:
                    continue
    ...
    # Har bir (YANGI, hali joylanmagan) natija uchun gol urgan futbolchi va daqiqasini
    # olamiz — allaqachon joylangan o'yinlar yuqorida chetlab o'tilgani uchun bu yerga
    # kelmaydi ham, ortiqcha so'rov ketmaydi.
...
def collect_upcoming_fixtures(
    leagues: list[dict], days_forward: int, already_posted_ids: set[str] = frozenset(), on_progress=None,
) -> list[dict]:
    today = date.today()
    fixtures = []
    for league in leagues:
