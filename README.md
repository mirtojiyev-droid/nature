...
- **Tabiat boti** — avvalgi `nature_channel_bot` loyihasidan boshlangan, lekin shundan
  ...
  bog'langan jadval, FAQAT Pixabay (Pexels olib tashlangan), avtomatik sifat nazorati
  (buzuq/bo'sh fayllarni aniqlash, muvaffaqiyatsiz nomzoddan keyingisiga o'tish), "pro"
  brendlash (video+rasmga joy nomi/kanal belgisi), va ixtiyoriy AI video generatsiyasi
...
**Video/rasm manba tartibi — Pixabay (asosiy) + Pexels (qo'shimcha, ixtiyoriy) +
Wikimedia (oxirgi zaxira)**: foydalanuvchi dastlab ikkala platformani qo'lda
solishtirib, Pixabay asosiy manba bo'lishini tanlagan edi — lekin keyinroq
Pexels'dagi videolar ham chiroyli chiqishini va Pixabay ba'zan (ayniqsa noyob joy
nomlari uchun) yetarli natija bermayotganini payqab, Pexels'ni QO'SHIMCHA (ikkinchi)
manba sifatida qaytarishni so'radi. Hozirgi tartib: **Pixabay birinchi sinaladi**,
agar `PEXELS_API_KEY` sozlangan bo'lsa **Pexels ikkinchi**, Wikimedia Commons
(kalitsiz) — oxirgi zaxira. `PIXABAY_API_KEY` **shart**, `PEXELS_API_KEY`
**ixtiyoriy** (sozlanmasa, bot shunchaki uni chetlab o'tadi). Har bir manba/so'rov
birikmasi uchun bir nechta nomzod ketma-ket sinaladi (sifat nazoratidan o'tmasa,
keyingisiga o'tiladi — pastga qarang).
...
- **"exceeded its memory limit" / "out of memory" (Render, Railway va h.k.)** —
  ...
  tekshiring (standart shunday — kripto/futbol bir vaqtda ishlamasin; **eslatma**:
  tabiat boti bu sozlamaga bog'liq emas, u har doim mustaqil ishlaydi). Ikkalasi ham
  yordam bermasa, serveringizning operativ xotirasini oshiring (masalan Render'da
  Standard tarifga o'ting).
